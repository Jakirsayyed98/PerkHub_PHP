...
<div class="bg-white p-4 sm:p-6 rounded-lg card-shadow">
    <h2 class="text-xl sm:text-2xl font-bold mb-6 text-gray-800">Edit Store</h2>
    <form id="storeForm">
        ...
        <div class="mt-6 flex space-x-4">
            <button type="submit" class="btn-primary text-white px-4 py-2 rounded-lg font-medium">Update Store</button>
    ...
    async function fetchStore() {
        ...
        try {
            ...
            if (result.status) {
                ...
                document.getElementById('affiliate_provider_id').value = result.data.affiliate_provider_id;
                document.getElementById('icon').value = result.data.icon || '';
                document.getElementById('logo').value = result.data.logo || '';
                document.getElementById('banner').value = result.data.banner || '';
                document.getElementById('about_store').value = result.data.about_store || '';
                document.getElementById('terms_and_conditions').value = result.data.terms_and_conditions || '';
                document.getElementById('label').value = result.data.label || '';
                document.getElementById('cashback').value = result.data.cashback;
                document.getElementById('active').value = result.data.active ? '1' : '0';
            } else {
    ...
    document.getElementById('storeForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (result.status) {
                ...
                errorDiv.textContent = result.message || 'Failed to update store';
                errorDiv.classList.remove('hidden');
            }
        } catch (error) {
            errorDiv.textContent = 'Error updating store';
            errorDiv.classList.remove('hidden');
        }
    });
