...
<script>
    ...
    async function fetchTickets(page = 1, search = '') {
        try {
            const response = await fetch(`/api/admin/tickets?page=${page}&search=${encodeURIComponent(search)}`, {
                headers: {
            ...
            if (result.status) {
                ...
                if (result.data.data.length === 0) {
                    tableBody.innerHTML = '<tr><td colspan="6" class="p-3 text-center text-sm sm:text-base">No tickets found</td></tr>';
                } else {
                    result.data.data.forEach(ticket => {
                        const row = document.createElement('tr');
                        row.innerHTML = `
                            <td class="p-3 text-sm sm:text-base">${ticket.id}</td>
                            <td class="p-3 text-sm sm:text-base">${ticket.user?.name ?? '-'}</td>
                            ...
                            <td class="p-3">
                                <a href="{{ route('admin.tickets.edit', '${ticket.id}') }}" class="text-blue-500 hover:underline text-sm sm:text-base">View</a>
                            </td>
                        `;
