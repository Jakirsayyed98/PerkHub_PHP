...
<div class="bg-white p-4 sm:p-6 rounded-lg card-shadow">
    <h2 class="text-xl sm:text-2xl font-bold mb-6 text-gray-800">Edit Affiliate Provider</h2>
    <form id="affiliateForm">
        ...
        <div class="mt-6 flex space-x-4">
            <button type="submit" class="btn-primary text-white px-4 py-2 rounded-lg font-medium">Update Provider</button>
    ...
    async function fetchAffiliate() {
        ...
        try {
            ...
            if (result.status) {
                ...
                document.getElementById('base_url').value = result.data.base_url;
                document.getElementById('callback_secret').value = result.data.callback_secret || '';
    ...
    document.getElementById('affiliateForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (result.status) {
                ...
                errorDiv.textContent = result.message || 'Failed to update provider';
                errorDiv.classList.remove('hidden');
            }
        } catch (error) {
            errorDiv.textContent = 'Error updating provider';
            errorDiv.classList.remove('hidden');
        }
    });
